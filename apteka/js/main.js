...
// Скролл
  ...
  // прыгающая корзина
    $('.shopping').mouseup(function(event){
        $(this).css('top', 1);
        $('.section_list li .shopping .small_popup.disease').css('top', 1);      
    }).mousedown(function(event){
        $(this).css('top', 2);
        $('.section_list li .shopping .small_popup.disease').css('top', 0);
    ...
    // подсказка товара внизу списка
    $('.scroll_box').each(function(){
        var $this = $(this),
            $resultsList = $('.results_list li', $this),
            $sectionListLi = $('.section_list li', $this),
            speed = 500,
            block = false,
            wrapper = $('.popup_bot').height(),
            offset = $(this).offset();

        $('.section_list li', $this).mouseenter(function(event){
            $('.section_list li:hover .small_popup.orange').stop(false, true).fadeIn(speed);

            var scrolBoxTop = $(this).closest('.scroll_box').offset().top,
                liTop = $(this).offset().top,
                popupHeight = $(this).find('.small_popup.orange').height(),
                distance = liTop - scrolBoxTop,
                liHeight = $(this).height(),
                distancePopup = distance + popupHeight + liHeight + 20,
                popupTop = $(this).find('.small_popup.orange').css('top');

                if(popupTop != undefined) {
                    $(this).slice(0, -2);
                }
            if (wrapper < distancePopup) {
                $(this).find('.small_popup.orange').css('top', -popupHeight-15);
                $(this).find('.popup_arrow').hide();
                $(this).find('.popup_arrow.bottom').show();

            }
        }).mouseleave(function(event){
            $('.small_popup.orange').filter(':visible').stop(false, true).fadeOut(speed);
        });
    });
...
// -----------------------------------------------------Корзина---------------------------------------------------------
    ...
    function showPhoto(){
        ...
        $('.map_box').on('click', '.popup_photo', function(){

        var $this = $(this),
            img = $this.parent().find('.popup_img'),
            speed = 300;

        if (photoBlock) return false; photoBlock = true;

        img.fadeToggle(speed, function(){
          photoBlock = false;
        });

        $this.toggleClass('active');

        var mapPopupHeight = $('.mapPopup').height() +10,
            mapHeight = $('#map').height(),
            offsetWrap = $('#map').offset().top,
            offsetObj = $(this).offset().top,
            offset = offsetObj - offsetWrap,
            distance = offset + mapPopupHeight,
            canvasTop = $('.ymaps-layers-pane').find('canvas').css('top'),
            canvasTopN = parseInt(canvasTop, 10),
            spacing = mapPopupHeight + canvasTopN;
        if( mapHeight < distance ){
            myMap.panTo([55.5013238525390362447091607116, 37.301323852539036], {flying: true} )
        }

        // var heightBaloon = $this.closest('.ymaps-b-balloon__content').height();
        // console.log(heightBaloon);
        // $this.closest('.ymaps-b-balloon__content > *').height(heightBaloon += 176);
...
var myMap;
    function init () {
        ...
        function makePlacemarker( number, coordinates, content, time ){
          var imgHref = '';

          if ( time == 'night' ){
            imgHref = 'ico/marker_night.png';
          }else{
            imgHref = 'ico/marker.png';
          }
        ...
        $('.map_list > li').each(function(){
            ...
            myCollection.add( makePlacemarker(  
                            ...
                            coords,
                            '<div class="small_popup mapPopup">\
                                    <h3 class="title_popup">'+ title +'</h3>'
                                    + bubleContent + 
                                    '<span class="popup_photo"></span>' +
                                    '<img src="uploads/map_popup_small.png" class="popup_img" alt="" />' +
                                    '<span class="popup_arrow green"></span>' +
        ...
        myCollection.events.add('click', function(event){
          ...
          var scrollPos = ( myCollection.indexOf(event.get('target')) );
          $(mapListli[scrollPos]).addClass('active').siblings('.active').removeClass('active');
    ...
    **/
    ...
    $(".tabsBox").each(function() {
        ...
        $('li', tabsControls).click(function(){
...
// Сортировка
    ...
    $('#popup_fone').on('click', function(event){
      $('.showBox').fadeOut(400);
    });
    ...
    $('.arise_box').each(function(event){
      var $this = $(this),
          ...
          pos = 0,
          block = false,
          slideLenght = ariseSliderLi.length;
      close.click(function(){
        $(this).closest($this).fadeOut(speed);
        $('#popup_block').fadeOut(speed);
        return false;
      })
      ...
      $('img', ariseSliderLi).click(function(){
          if (block) return false; block = true;
          pos++;
          if(slideLenght <= pos){
              pos = 0;
          }
          $('a', paginator).eq(pos).addClass('active').siblings('.active').removeClass('active');
          ariseSliderLi.eq(pos).fadeIn(speed, function(){block = false}).css('position','relative').siblings().fadeOut(speed).css('position','absolute');
          return false;
      })
...
// слайдер главная предпросмотр
    $('.previewSlider').each(function(){
        ...
        var $this = $(this),
            $controlBlock = $('.slides_controls', $this),
            $control = $('.slides_controls li', $this),
            $viewImgBlock = $('.slides', $this),
            $viewImg = $('.slides li', $this),
            liWidth = $('.slides li', $this).first().outerWidth(),
            controlLength = $control.length,
            block = false,
            pos = 0,
            idx = 0,
            count = $('.previewSlider .slides_controls li').length,
            slides = '',
            flag_moveSpeed = 1000;
        ...
        $viewImg.mouseenter(function(){
            if( block ) return false; 
            block = true;
            $(this).children('span').fadeIn(500, function(){
                block = false;
            });
        }); 
    ...
    $('.bannerWrapper').each(function(){
        ...
        var $this = $(this),
            $bannerLi = $('.bannerWrapper li');

        $bannerLi.mouseenter(function(){
            if( block ) return false; 
            block = true;
            $(this).children('span').fadeIn(500, function(){
                block = false;
    ...
    // попап информации в поисках
    $('.bg_wrap_right').each(function(){
        var $this = $(this),
            speed = 500;

        $('.titile_middle.popup').mouseenter(function(event){
            $('.titile_middle .small_popup').stop(false, true).fadeIn(speed);
        }).mouseleave(function(event){
            $('.titile_middle .small_popup').stop(false, true).fadeOut(speed);
        });
    });

    // каталог выбор производителя
    // $('#middle').each(function(){
    //     var $this = $(this),
    //         $choice = $('.product_filters_choice'),
    //         $wrapper = $('.product_filters');

    //     $wrapper.on('click, change', function(){
    //         var val = $(this).find('.big').next('.sbHolder').find('.sbSelector').text();
    //         $choice.text(val);
    //     })
    ...
    $(window).on('scroll', function(){
        ...
        if(scrollTop > 0){
          ...
          if(scrollBlock) return; scrollBlock = true;
    ...
    // поиск удаление всего текста сразу бэкспейсом
    var searchBlock = false;
    $('.simple_search').each(function(){
        var $this = $(this),
            input = $('.searchInp', $this),
            speed = 500,
            block = false;
        ...
        input
          .on('keydown', function(event){
            ...
            if (event.which == 8){
              backTimeId = setTimeout(function() {
                input.val('');
              }, 500);
            }
          })
          ...
          .on('click', function(){
            if(block) return false;
            block = true;
            $(this).closest('.simple_search').find('.search_popup').fadeOut(speed, function(){
              block = false;
            });
          })

        $('body').on('click', function(){
          $('.simple_search:visible', $this).remove();
          $(this).find('.simple_search .search_popup').fadeIn(speed);
        })
    });
    ...
    $('.navigate.call').each(function(){
        var $this = $(this),
            number = $('.call_number').text(),
            text = $('.call_text').text();

        $('.call_number').mouseenter(function(event){
            $(this).text(text);
        }).mouseleave(function(event){
            $(this).text(number);
        });
    ...
    $('.basket_box').each(function(){
        var $this = $(this),
            wrap = $('.orderScroll', $this),
            wrapHeight = wrap.height(),
            all = $('.mCSB_container', $this)[0],            
            speed = 500;

        $('.order_list li').mouseenter(function(event){
            var liHeight = $(this).height(),
                liTop = $(this).position().top,
                allHeight = all.scrollHeight;
                // console.log(wrapHeight, liHeight, liTop, allHeight,$('.mCSB_container', $this)[0] )
                // console.log(allHeight - liTop - liHeight)
            if ( wrapHeight < liTop + liHeight ){
              console.log(-liTop +50)
              $('.scroll_box', $this).mCustomScrollbar("scrollTo", liTop -liHeight -40,{
                  scrollInertia:1000
              });
                // $('.mCSB_container').animate({
                //   'top': -liTop+liHeight +50
                // }, speed);
                // $('.mCSB_dragger').animate({
                //   'top': +30
                // }, speed);
            }
        })
    ...
    var defaultText = $('.inBox').text();
    $('.inBox').mouseenter(function(event){
        var $this = $(this),
            change = $('.change_inBox').text();
        $this.text(change);
    }).mouseleave(function(event){
        $('.inBox').text(defaultText);
    });

    // регестрация всплыв инфо
    $('.registration_ico').mouseenter(function(){
        $(this).next().stop(false, true).fadeIn(500);
    }).mouseleave(function(){
        $(this).next().stop(false, true).fadeOut(500);
    });
